...
// Bloquer numéros et emails dans les messages
function containsContact(text) {
  // Numéros de téléphone (toutes formes)
        ...
        <div className="w-72 border-r border-slate-100 overflow-y-auto shrink-0">
          {conversations.length === 0 ? (
            ...
            <button key={conv.id} onClick={() => setSelected(conv)}
              ...
              <div className="text-sm font-semibold text-slate-800 line-clamp-1">{conv.annonce_titre || 'Annonce'}</div>
        ...
        <div className="flex-1 flex flex-col">
          {selected ? (
            <>
              ...
              {/* Input */}
              <form onSubmit={sendMsg} className="px-4 py-3 border-t border-slate-100">
                {contactError && (
                  <p className="text-xs text-red-500 mb-2">⚠️ Les numéros de téléphone et emails ne sont pas autorisés dans les messages.</p>
                )}
